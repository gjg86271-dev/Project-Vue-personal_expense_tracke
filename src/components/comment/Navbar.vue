      ...
      <div v-if="notifOpen" ref="notifMenuRef" class="notif-panel" :style="notifStyle" @click.stop>
        ...
        <div class="notif-head">
          ...
          <div class="notif-actions">
            <button v-if="unreadCount > 0" class="btn-notif-action" @click="markAllRead">
              សម្គាល់ទាំងអស់
            </button>
            <!-- ✅ Clear all button -->
            <button v-if="alerts.length > 0" class="btn-notif-action btn-notif-action--clear" @click="clearAll">
              សម្អាត់ទាំងអស់
            </button>
          </div>
        </div>
...
// ✅ Persist only "read" state (not dismissed/cleared)
const readSet     = ref(new Set(JSON.parse(localStorage.getItem('notif-read') || '[]')))
// ✅ Persist "cleared" alert IDs so they stay hidden until data changes
const clearedSet  = ref(new Set(JSON.parse(localStorage.getItem('notif-cleared') || '[]')))
...
function persistCleared() { localStorage.setItem('notif-cleared', JSON.stringify([...clearedSet.value])) }
...
// ✅ Clear all — hides all current alerts without deleting data
function clearAll() {
  alerts.value.forEach(a => {
    clearedSet.value.add(a.id)
    readSet.value.add(a.id)
  })
  persistCleared()
  persistRead()
  alerts.value = []
...
// ── Core alert computation (pure — no API calls) ───────────────────────────
function computeAlerts() {
  const now          = new Date()
  const month        = now.getMonth() + 1
  const year         = now.getFullYear()

  // get budgets from store (already fetched by BudgetView)
  const allBudgets   = budgetStore.budgets ?? []
  const budgets      = allBudgets.filter(
    b => b.month === month && b.year === year && b.category?.type === 'EXPENSE'
  )
  if (!budgets.length) { alerts.value = []; return }

  // get transactions from store (already fetched by BudgetView)
  ...
  for (const b of budgets) {
    ...
    // ✅ If percent changed (e.g. went from warning→danger), remove from cleared
    // so the alert reappears with the new severity
    const existing = alerts.value.find(a => a.id === id)
    if (clearedSet.value.has(id) && existing?.level !== level) {
      clearedSet.value.delete(id)
      persistCleared()
    }

    // ✅ Skip alerts the user already cleared (unless severity changed above)
    if (clearedSet.value.has(id)) continue

    result.push({
      id, level, message, category: catName,
      spent, limit, percent,
      read: readSet.value.has(id),
    })
  }

  // Prune stale read/cleared IDs
  for (const id of [...readSet.value])   { if (!activeIds.has(id)) readSet.value.delete(id) }
  for (const id of [...clearedSet.value]){ if (!activeIds.has(id)) clearedSet.value.delete(id) }
  persistRead()
  persistCleared()
...
// ── Initial fetch (first load only) ───────────────────────────────────────
async function fetchBudgetAlerts() {
  notifLoading.value = true
  try {
    // Fetch if store is empty (navbar loaded before BudgetView)
    if (!budgetStore.budgets?.length) {
      const res = await api.get('budgets')
      budgetStore.budgets = res.data?.data?.items ?? []
    }
    if (!transactionStore.transactions?.length) {
      const res = await api.get('transactions')
      transactionStore.transactions = res.data?.data?.items ?? []
    }
  } catch (e) {
    console.error('budget alerts fetch failed', e)
  } finally {
...
// ✅ Watch store data — recompute instantly when transactions or budgets change
watch(
  () => [budgetStore.budgets, transactionStore.transactions],
  () => computeAlerts(),
  { deep: true }
)
...
.btn-notif-action {
  ...
  font-family: var(--font-khmer); padding: 0;
  transition: opacity .15s;
}
.btn-notif-action:hover { opacity: .7; }

/* ✅ Clear button — distinct color */
.btn-notif-action--clear {
...
.profile-btn {
  ...
  border: 1px solid transparent; background: transparent;
  cursor: pointer; color: rgba(255,255,255,.9); transition: var(--transition);
}
.profile-btn:hover {
